...
export class LockThreadUseCase {
  ...
  async execute(courseId: string, threadId: string, isLocked: boolean, actorId: string, actorRole: UserRole): Promise<ForumThreadEntity> {
    if (actorRole !== UserRole.TEACHER && actorRole !== UserRole.ADMIN) {
      throw new ForbiddenException('Only teachers and admins can lock threads');
    }
    ...
    return this.forumRepo.updateThread(threadId, { isLocked });
  }
}
