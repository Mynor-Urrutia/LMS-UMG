...
export class UnpublishCourseUseCase {
  ...
  async execute(id: string, requesterId: string, requesterRole: UserRole): Promise<void> {
    ...
    if (!isOwner && !isAdmin) throw new ForbiddenException('Only the course owner or an admin can unpublish this course');

    if (course.status !== CourseStatus.PUBLISHED) {
      throw new UnprocessableEntityException('Only published courses can be unpublished');
    }

    await this.courseRepo.updateStatus(id, CourseStatus.DRAFT);
  }
}
