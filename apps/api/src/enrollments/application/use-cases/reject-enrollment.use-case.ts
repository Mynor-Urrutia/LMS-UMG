...
export class RejectEnrollmentUseCase {
  ...
  async execute(
    ...
    if (!isOwner && !isAdmin) {
      throw new ForbiddenException('Only the course owner or an admin can reject enrollments');
    }

    if (enrollment.status !== EnrollmentStatus.PENDING) {
      throw new UnprocessableEntityException('Only PENDING enrollments can be rejected');
    }

    await this.enrollmentRepo.updateStatus(enrollmentId, EnrollmentStatus.REJECTED);
  }
}
